...
def test_deleting_user_cascades_authentication_records(
    ...
    refresh_count = db_session.scalar(select(func.count()).select_from(RefreshSession))
    reset_count = db_session.scalar(select(func.count()).select_from(PasswordResetToken))
    profile_count = db_session.scalar(select(func.count()).select_from(UserProfile))
    settings_count = db_session.scalar(select(func.count()).select_from(UserSettings))
